...
function Module2() {
  ...
  const [newInvoice, setNewInvoice] = useState({
    ...
    netAmount: "",
    issueDate: "",
    paid: false, // Nowa właściwość do śledzenia opłacenia faktury
  });

  const [filterPaid, setFilterPaid] = useState(null); // Filtr faktur opłaconych/nieopłaconych
          ...
          <ul className="module2__invoice-list">
            {filteredInvoices.map((invoice, index) => (
              <li key={index} className="module2__invoice-item">
                Numer faktury: {invoice.number}, Kwota netto:{" "}
                {invoice.netAmount}, Data wystawienia: {invoice.issueDate},
                Opłacona: {invoice.paid ? "Tak" : "Nie"}
              </li>
            ))}
          </ul>
        </div>
